...
export function Toolbar() {
  ...
  return (
    <>
      <div
        className="fixed bottom-6 left-1/2 z-50 flex items-center gap-1 rounded-2xl border border-slate-700/50 bg-slate-800/80 p-2 shadow-2xl backdrop-blur-xl transition-all duration-700 ease-out"
        style={{
      ...
      >
        {TOOLS.map((tool) => {
          ...
          return (
            ...
            >
              ...
              <span className="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-700 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity">
                {tool.label}
              </span>
            </button>
          );
        ...
        <div className="relative">
          ...
          >
            <Film size={20} strokeWidth={isMediaMenuOpen || isMovieSearchOpen ? 2.5 : 2} />
            <span className="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-700 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap">
              Media & Movies
            </span>
          </button>

          {isMediaMenuOpen && (
            <div className="absolute bottom-full mb-4 left-1/2 -translate-x-1/2 w-48 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden flex flex-col z-50">
              ...
              <button
                onClick={() => {
                  ...
                  // Handle YouTube links specifically for embedding
                  ...
                  const iframeEl = {
                    ...
                    y: worldPt.y - 225,
                    width: 800,
                    height: 450, // Better aspect ratio for YouTube
                    url: embedUrl,
                    title: title,
                    poster: posterUrl,
                    style: {}
        ...
        <button 
          ...
          className="p-3 rounded-xl text-slate-400 hover:bg-slate-700/50 hover:text-slate-100 transition-colors"
          title="Undo"
        >
          <Undo2 size={20} />
        </button>
